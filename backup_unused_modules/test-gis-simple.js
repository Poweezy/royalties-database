...
async function testGisDashboardEnhancement() {
  ...
  try {
    ...
    // Test icon creation
    console.log(
      `✅ Custom icons created: ${Object.keys(dashboard.icons).length} types`,
    );
    console.log(
      `   Icons available: ${Object.keys(dashboard.icons).join(", ")}`,
    );

    // Test layers
    console.log(
      `✅ Enhanced layers created: ${Object.keys(dashboard.layers).length} layers`,
    );
    console.log(
      `   Layers available: ${Object.keys(dashboard.layers).join(", ")}`,
    );
    ...
    testMines.forEach((mine) => {
      ...
      const icon = dashboard.selectIcon(
        mine.name,
        mineralType,
        mine.name.toLowerCase().includes("quarry"),
      );
      console.log(
        `✅ ${mine.name} → ${mineralType} → ${icon.options.iconUrl.includes("red") ? "red" : icon.options.iconUrl.includes("blue") ? "blue" : icon.options.iconUrl.includes("black") ? "black" : icon.options.iconUrl.includes("orange") ? "orange" : icon.options.iconUrl.includes("green") ? "green" : "default"} marker`,
      );
    });
    ...
    if (
      popupContent.includes("Test Mine") &&
      popupContent.includes("Gold") &&
      popupContent.includes("popup-actions")
    ) {
    ...
    console.log(
      "✅ Geographic Overlays: Concessions, districts, protected areas, infrastructure",
    );
    console.log(
      "✅ Analysis Tools: Measurement, drawing, and spatial query capabilities",
    );
    console.log(
      "✅ Enhanced Features: Export, geocoding, coordinate display, custom legend",
    );
