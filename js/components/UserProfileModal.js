...
export class UserProfileModal {
  ...
  getOnboardingTabHTML() {
    const onboardingStatus = this.userManager.getUserOnboardingStatus(
      this.currentUser.id,
    );
    ...
    return `
            <div class="onboarding-tab">
                ...
                <div class="onboarding-steps">
                    ${onboardingStatus.template.steps
                      .map((step, index) => {
                        const isCompleted = onboardingStatus.completed.includes(
                          step.id,
                        );
                        return `
  ...
  setupModalEventListeners(modal) {
    ...
    const securityNotificationBtn = modal.querySelector(
      "#send-security-notification",
    );
    if (securityNotificationBtn) {
      securityNotificationBtn.addEventListener("click", () =>
  ...
  async loadActivityData() {
    try {
      const activity = await this.userManager.getUserActivitySummary(
        this.currentUser.id,
      );

      // Update stats
      document.getElementById("login-count").textContent =
        activity.loginAttempts.filter((a) => a.successful).length;
  ...
  async loadSecurityData() {
    try {
      const recentAttempts = await userSecurityService.getRecentLoginAttempts(
  ...
  updateActivityTimeline(activity) {
    ...
    const allActivities = [
      ...activity.loginAttempts.map((a) => ({
        ...a,
        type: "login",
        icon: "fa-sign-in-alt",
      })),
      ...activity.securityEvents.map((e) => ({
        ...e,
        type: "security",
        icon: "fa-shield-alt",
      })),
    ...
    timeline.innerHTML = allActivities
      .map(
        (activity) => `
            <div class="timeline-item">
  ...
  getActivityDetails(activity) {
    if (activity.type === "login") {
      ...
      return activity.details
        ? JSON.stringify(JSON.parse(activity.details))
        : "";
    }
    return "";
  }
  ...
  async saveProfile() {
    try {
      const profileData = {
        ...
        twoFactorEnabled:
          document.getElementById("profile-2fa")?.checked || false,
        forcePasswordChange:
          document.getElementById("profile-force-password")?.checked || false,
        passwordPolicy:
          document.getElementById("profile-password-policy")?.value ||
          "default",
        profile: {
  ...
  async resetPassword() {
    ...
    try {
      ...
      console.log(
        `Password reset for ${this.currentUser.username}. Temporary password: ${tempPassword}`,
      );
  ...
  async toggleStepCompletion(stepId, completed) {
    try {
      await this.userManager.updateOnboardingProgress(
        this.currentUser.id,
        stepId,
        completed,
      );
      ...
      if (this.activeTab === "onboarding") {
        document.getElementById("onboarding-tab").innerHTML =
          this.getOnboardingTabHTML();
      }
    } catch (error) {
