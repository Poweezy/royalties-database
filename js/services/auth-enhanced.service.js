...
class EnhancedAuthService {
  ...
  async enhancedLogin(
    ...
    try {
      ...
      if (
        this.config.twoFactorEnabled &&
        this.isUserTwoFactorEnabled(username)
      ) {
        if (!twoFactorCode) {
      ...
      this.recordSecurityEvent("login_failed", username, {
        error: error.message,
      });
      this.saveStoredData();
      throw error;
  ...
  createEnhancedSession(username, rememberMe = false) {
    ...
    const session = {
      ...
      expiresAt:
        now +
        (rememberMe ? 30 * 24 * 60 * 60 * 1000 : this.config.sessionTimeout), // 30 days if remember me, else session timeout
      deviceFingerprint,
  ...
  checkAccountLockout(username) {
    ...
    if (
      count >= this.config.maxFailedAttempts &&
  ...
  cleanupExpiredSessions() {
    ...
    expiredSessions.forEach((sessionId) => {
      ...
      this.recordSecurityEvent("session_expired", session.username, {
        sessionId,
      });
    ...
    document.addEventListener("visibilitychange", () => {
      if (document.hidden) {
        this.recordSecurityEvent(
          "tab_hidden",
          this.baseAuthService.currentUser?.username,
        );
      } else {
        this.recordSecurityEvent(
          "tab_visible",
