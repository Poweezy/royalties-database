...
class RoyaltyValidationService {
    constructor() {
        ...
        this.mineralLimits = {
            ...
            'Iron Ore': { minVolume: 1, maxVolume: 50000, minTariff: 20, maxTariff: 200 },
            'Gold': { minVolume: 0.01, maxVolume: 1000, minTariff: 1000, maxTariff: 50000 },
            'Diamond': { minVolume: 0.001, maxVolume: 100, minTariff: 5000, maxTariff: 100000 },
            'Quarried Stone': { minVolume: 1, maxVolume: 200000, minTariff: 5, maxTariff: 50 },
            'Gravel': { minVolume: 1, maxVolume: 150000, minTariff: 5, maxTariff: 40 },
            'Sand': { minVolume: 1, maxVolume: 200000, minTariff: 3, maxTariff: 30 },
            'Limestone': { minVolume: 1, maxVolume: 100000, minTariff: 8, maxTariff: 60 }
        };
    }
    ...
    validateRoyaltyRecord(record) {
        ...
        const requiredFields = ['entity', 'mineral', 'volume', 'tariff', 'paymentDate'];
        ...
        [entityValidation, mineralValidation, volumeValidation, tariffValidation, dateValidation]
            .forEach(validation => {
        ...
        if (businessRuleValidation.errors) errors.push(...businessRuleValidation.errors);
        if (businessRuleValidation.warnings) warnings.push(...businessRuleValidation.warnings);

        // Cross-field validation
        const crossFieldValidation = this.validateCrossFields(record);
        if (crossFieldValidation.errors) errors.push(...crossFieldValidation.errors);
        if (crossFieldValidation.warnings) warnings.push(...crossFieldValidation.warnings);
    ...
    validateEntity(entity) {
        ...
        const knownEntities = [
            'Kwalini Quarry', 'Maloma Colliery', 'Mbabane Quarry', 
            'Ngwenya Mine', 'Sidvokodvo Quarry', 'Piggs Peak Quarry',
            'Matsapha Stone', 'Havelock Mine'
        ];

        if (!knownEntities.some(known => 
    ...
    validateMineral(mineral) {
        ...
        if (!validMinerals.includes(mineral)) {
            warnings.push(`Mineral type '${mineral}' is not in the standard list. Valid types: ${validMinerals.join(', ')}`);
        }
    ...
    validateVolume(volume, mineral) {
        ...
        if (numericVolume > this.businessRules.volume.max) {
            errors.push(`Volume cannot exceed ${this.businessRules.volume.max.toLocaleString()}`);
        }
        ...
        if (mineral && this.mineralLimits[mineral]) {
            ...
            if (numericVolume < limits.minVolume) {
                warnings.push(`Volume ${numericVolume} is below typical minimum for ${mineral} (${limits.minVolume})`);
            }
            
            if (numericVolume > limits.maxVolume) {
                warnings.push(`Volume ${numericVolume} is above typical maximum for ${mineral} (${limits.maxVolume})`);
            }
        }
        ...
        if (decimalPlaces > this.businessRules.volume.precision) {
            warnings.push(`Volume has more than ${this.businessRules.volume.precision} decimal places`);
        }
    ...
    validateTariff(tariff, mineral) {
        ...
        if (numericTariff > this.businessRules.tariff.max) {
            errors.push(`Tariff cannot exceed E${this.businessRules.tariff.max.toLocaleString()}`);
        }
        ...
        if (mineral && this.mineralLimits[mineral]) {
            ...
            if (numericTariff < limits.minTariff) {
                warnings.push(`Tariff E${numericTariff} is below typical range for ${mineral} (E${limits.minTariff} - E${limits.maxTariff})`);
            }
            
            if (numericTariff > limits.maxTariff) {
                warnings.push(`Tariff E${numericTariff} is above typical range for ${mineral} (E${limits.minTariff} - E${limits.maxTariff})`);
            }
        }
    ...
    validateBusinessRules(record) {
        ...
        if (record.volume && record.tariff) {
            const expectedPayment = parseFloat(record.volume) * parseFloat(record.tariff);
            const actualPayment = parseFloat(record.royaltyPayment || expectedPayment);
        ...
        // Status validation
        if (record.status) {
            const validStatuses = ['Draft', 'Pending', 'Paid', 'Overdue', 'Disputed', 'Cancelled'];
            if (!validStatuses.includes(record.status)) {
                errors.push(`Invalid status '${record.status}'. Valid options: ${validStatuses.join(', ')}`);
            }
    ...
    validateCrossFields(record) {
        ...
        if (record.volume && record.tariff && record.mineral) {
            ...
            if (volume > 10000 && tariff > 50) {
                warnings.push('High volume orders typically qualify for reduced tariff rates');
            }
            
            // Small volume with very low tariff might be suspicious
            if (volume < 10 && tariff < 5) {
                warnings.push('Very small volume with very low tariff - please verify amounts');
            }
        }
    ...
    async validateAgainstContract(record, contracts) {
        ...
        if (!matchingContract) {
            warnings.push(`No active contract found for ${record.entity} - ${record.mineral}`);
            return { errors, warnings };
        }
        ...
        if (matchingContract.startDate) {
            ...
            if (paymentDate < contractStart) {
                errors.push(`Payment date is before contract start date (${matchingContract.startDate})`);
            }
        }
        ...
        if (matchingContract.endDate) {
            ...
            if (paymentDate > contractEnd) {
                errors.push(`Payment date is after contract end date (${matchingContract.endDate})`);
            }
        }

        // Validate calculation method matches
        if (matchingContract.calculationType === 'fixed' && 
