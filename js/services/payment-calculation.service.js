...
class PaymentCalculationService {
    ...
    async performBaseCalculation(recordData, calculation, options) {
        ...
        calculation.calculationRules.appliedRules = calculation.baseCalculation.appliedRules || [];
    }
    ...
    async calculateTiered(recordData, options) {
        const contract = options.contract || this.getContractData(recordData.contractId);
        const tiers = contract?.calculationParams?.tiers || this.getDefaultTiers(recordData.mineral);
        ...
        for (const tier of tiers) {
            ...
            breakdown[`Tier ${tier.from}-${tier.to || '∞'} @ ${tier.rate}`] = tierAmount;
        }
        ...
        return {
            ...
            appliedRules: appliedTiers.map(t => `${t.volume} tons @ ${t.rate} = ${t.amount}`),
            breakdown: breakdown
        };
    }
    ...
    async calculateSlidingScale(recordData, options) {
        const contract = options.contract || this.getContractData(recordData.contractId);
        const scales = contract?.calculationParams?.scales || this.getDefaultScales(recordData.mineral);
        ...
        for (const scale of scales) {
            if (commodityPrice.current >= scale.priceFrom && 
        ...
        const basePrice = contract?.calculationParams?.basePrice || commodityPrice.baseline;
    ...
    async calculateAdValorem(recordData, options) {
        const marketValue = recordData.marketValue || this.calculateMarketValue(recordData);
    ...
    async calculatePercentage(recordData, options) {
        const grossValue = recordData.grossValue || this.calculateGrossValue(recordData);
        const percentageRate = recordData.percentageRate || 0.10; // 10% default
    ...
    async calculateHybrid(recordData, options) {
        const hybridConfig = options.hybridConfig || this.getDefaultHybridConfig(recordData.mineral);
        ...
        for (const component of hybridConfig.components) {
            ...
            const componentCalc = await this.calculationMethods[component.method](componentData, options);
            ...
            appliedRules.push(`${component.method} (${(component.weight * 100)}%): ${componentAmount.toFixed(2)}`);
    ...
    async calculatePenalties(recordData, calculation, options) {
        ...
        const daysPastDue = Math.max(0, Math.ceil((today - paymentDate) / (1000 * 60 * 60 * 24)));
        ...
        if (daysPastDue <= 0 || recordData.status === 'Paid') {
            calculation.penalties.appliedRules.push('No penalties - payment not overdue');
            return;
        }
        ...
        if (this.penaltyRates.compound && daysPastDue > 30) {
            ...
            penaltyAmount = baseAmount * Math.pow(1 + penaltyRate, compoundPeriods) - baseAmount;
    ...
    async calculateInterest(recordData, calculation, options) {
        const paymentDate = new Date(recordData.paymentDate);
        const today = new Date();
        const daysLate = Math.max(0, Math.ceil((today - paymentDate) / (1000 * 60 * 60 * 24)));
        ...
        if (daysLate <= gracePeriod || recordData.status === 'Paid') {
            calculation.interest.appliedRules.push(`No interest - within ${gracePeriod} day grace period`);
            return;
        }

        const interestPeriod = daysLate - gracePeriod;
        const annualRate = recordData.status === 'Disputed' ? 
        ...
        const interestAmount = baseAmount * (Math.pow(1 + dailyRate, interestPeriod) - 1);
    ...
    async applyCurrencyAdjustments(recordData, calculation, options) {
        ...
        if (targetCurrency === baseCurrency) {
            calculation.adjustments.appliedRules.push('No currency adjustment needed');
            return;
        }
    ...
    async validateCalculation(recordData, calculation, options) {
        ...
        if (calculation.total > 1000000) { // 1M threshold
        ...
        if (calculation.baseCalculation.baseAmount < minAmount) {
            calculation.warnings.push(`Payment below minimum threshold for ${recordData.mineral}`);
        }
    }

    // Utility methods
    getCommodityPrice(mineral) {
        return this.commodityPrices[mineral] || { 
            current: 50, 
            baseline: 50, 
            unit: 'USD/ton' 
        };
    ...
    calculateGrossValue(recordData) {
        const unitPrice = recordData.unitPrice || this.getCommodityPrice(recordData.mineral).current;
    ...
    getDefaultScales(mineral) {
        ...
        return [
            ...
            { priceFrom: 101, priceTo: null, rate: 0.10 }
        ];
    }
