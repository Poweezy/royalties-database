...
class App {
  ...
  async initializeDashboard() {
    try {
      ...
      const totalRoyalties = royaltyRecords.reduce(
        (sum, record) => sum + record.royaltyPayment,
        0,
      );

      const royaltyData = {
        totalRoyalties: totalRoyalties,
        paidRoyalties: royaltyRecords
          .filter((r) => r.status === "Paid")
          .reduce((sum, r) => sum + r.royaltyPayment, 0),
        pendingRoyalties: royaltyRecords
          .filter((r) => r.status !== "Paid")
          .reduce((sum, r) => sum + r.royaltyPayment, 0),
        yearlyTarget: 3500000.0,
      };
  ...
  setupEventListeners() {
    ...
    document
      ...
      ?.addEventListener("click", () => {
        document.getElementById("compose-message-container").style.display =
          "block";
      });

    document
      .getElementById("close-compose-form")
      ?.addEventListener("click", () => {
        document.getElementById("compose-message-container").style.display =
          "none";
      });
    ...
    document
      .getElementById("compose-message-form")
      ?.addEventListener("submit", (e) => {
        e.preventDefault();
  ...
  #setupUserManagementListeners() {
    ...
    const addUserFormContainer = document.getElementById(
    ...
    const showForm = (isEditMode = false, user = null) => {
      if (isEditMode && user) {
        formTitle.innerHTML =
          '<i class="fas fa-user-edit" aria-label="Edit User icon"></i> Edit User Account';
        createUserBtn.innerHTML =
          '<i class="fas fa-save" aria-label="Save icon"></i> Update User';
        this.#populateUserForm(user);
        addUserForm.dataset.editingId = user.id;
      } else {
        formTitle.innerHTML =
          '<i class="fas fa-user-plus" aria-label="Add New User icon"></i> Add New User Account';
        createUserBtn.innerHTML =
    ...
    addUserForm?.addEventListener("submit", (e) => {
      ...
      const editingId = e.target.dataset.editingId
        ? parseInt(e.target.dataset.editingId, 10)
        : null;
      const userData = {
      ...
      if (editingId) {
        ...
        this.notificationManager.show(
          `User '${userData.username}' updated successfully.`,
          "success",
        );
      } else {
        // Add new user
    ...
    const bulkStatusApplyBtn = document.getElementById(
      "bulk-apply-status-change",
    );
    ...
    const toggleBulkActionsContainer = () => {
      const selectedCheckboxes = userTable.querySelectorAll(
    ...
    bulkDeleteBtn?.addEventListener("click", () => {
      ...
      const userIdsToDelete = Array.from(selectedCheckboxes).map((cb) =>
      ...
      if (
        ...
        confirm(
          `Are you sure you want to delete ${userIdsToDelete.length} selected user(s)?`,
        )
      ) {
        this.userManager.bulkDeleteUsers(userIdsToDelete);
        this.notificationManager.show(
          `${userIdsToDelete.length} user(s) deleted successfully.`,
          "success",
        );
        toggleBulkActionsContainer();
    ...
    bulkStatusApplyBtn?.addEventListener("click", () => {
      ...
      const userIdsToUpdate = Array.from(selectedCheckboxes).map((cb) =>
        parseInt(cb.value, 10),
      );
      ...
      if (
        userIdsToUpdate.length > 0 &&
        confirm(
          `Are you sure you want to change the status to '${newStatus}' for ${userIdsToUpdate.length} selected user(s)?`,
        )
      ) {
        this.userManager.bulkUpdateUserStatus(userIdsToUpdate, newStatus);
        this.notificationManager.show(
          `Status for ${userIdsToUpdate.length} user(s) updated to '${newStatus}'.`,
          "success",
        );
        toggleBulkActionsContainer();
        selectAllCheckbox.checked = false;
    ...
    tableHeaders.forEach((header) => {
      header.addEventListener("click", () => {
        ...
        tableHeaders.forEach((th) => {
          if (th === header) {
            th.classList.add(
              this.userManager.sortDirection === "asc"
                ? "sorted-asc"
                : "sorted-desc",
            );
            th.classList.remove(
  ...
  #setupDashboardListeners() {
    ...
    customizeDropdown?.addEventListener("click", (e) => {
      e.stopPropagation();
      const isExpanded =
        customizeDropdown.getAttribute("aria-expanded") === "true";
    ...
    dropdownMenu?.addEventListener("click", (e) => {
      ...
      if (action === "toggle-widget") {
        ...
        this.notificationManager.show(
          `Toggled ${widget} widget visibility`,
          "info",
        );
      }
    });
    ...
    const activeEntitiesCard = document.querySelector(
      ".metric-card:nth-child(2)",
    );
    if (activeEntitiesCard) {
      activeEntitiesCard.addEventListener("click", () =>
        this.#showActiveEntitiesModal(),
      );
    }

    const closeActiveEntitiesModalBtn = document.getElementById(
      "close-active-entities-modal-btn",
    );
    ...
    const pendingApprovalsCard = document.querySelector(
    ...
    const chartControlButtons = document.querySelectorAll(
      ".chart-btn[data-chart-id]",
    );
    ...
    const exportButtons = document.querySelectorAll(
      ".export-chart-btn[data-chart-id]",
    );
    ...
    const bulkRoyaltyUpdateForm = document.getElementById(
      "bulk-royalty-update-form",
    );
    if (bulkRoyaltyUpdateForm) {
      bulkRoyaltyUpdateForm.addEventListener("submit", (e) => {
        ...
        this.notificationManager.show(
          `Royalty rate for ${mineral} updated to E${newRate}`,
          "success",
        );
      });
    }
    ...
    const addClickListener = (selector, targetRoute, context = null) => {
      ...
      if (element) {
        ...
        element.addEventListener("click", () =>
          this.navigate(targetRoute, context),
        );
      }
    };
  ...
  handleDashboardFilterChange(metric, period) {
    ...
    if (metric === "royalties") {
      document.getElementById("total-royalties").textContent =
        `E ${data.totalRoyalties.toLocaleString()}`;
      ...
      document.getElementById("royalties-progress").style.width =
        `${data.progress}%`;
  ...
  #renderAdminAuditLog() {
    ...
    const mockLog = [
      {
        timestamp: "2025-08-27 10:00",
        user: "admin",
        action: "LOGIN",
        details: "Successful login from IP 192.168.1.1",
      },
      {
        timestamp: "2025-08-27 10:05",
        user: "admin",
        action: "UPDATE_RATE",
        details: "Bulk update for Coal to E15.50",
      },
      {
        timestamp: "2025-08-27 10:10",
        user: "finance_user",
        action: "VIEW_REPORT",
        details: "Generated Q2 summary report",
      },
    ];

    auditLogBody.innerHTML = mockLog
      .map(
        (entry) => `
            <tr>
  ...
  updateDashboardMetrics(royaltyData, entityData, complianceData) {
    ...
    if (pendingApprovalsEl) {
      pendingApprovalsEl.textContent = (
        entityData.totalEntities - entityData.activeEntities
      ).toString();
    }
    ...
    if (royaltiesProgress) {
      const progressPercent = (
        (royaltyData.totalRoyalties / royaltyData.yearlyTarget) *
        100
      ).toFixed(1);
  ...
  updateLeaderboards() {
    ...
    const overdueEntitiesList = document.getElementById(
      "overdue-entities-list",
    );
    ...
    if (topEntitiesList) {
      ...
      topEntitiesList.innerHTML = topEntities
        .map((e) => `<li>${e.name}<span>${e.amount}</span></li>`)
        .join("");
    }
    ...
    if (overdueEntitiesList) {
      ...
      overdueEntitiesList.innerHTML = overdueEntities
        .map((e) => `<li>${e.name}<span>${e.days} days overdue</span></li>`)
        .join("");
    }
  }
  ...
  renderMessageHistory() {
    ...
    if (tableBody) {
      tableBody.innerHTML = messageHistory
        .map(
          (msg) => `
                <tr>
                    <td>${msg.date}</td>
