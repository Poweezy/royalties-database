...
export class ChartManager {
  ...
  #metricData = {
    royalties: {
      2024: {
        ...
        forecast: "E 1,200,000.00",
        yoyGrowth: 15.8,
      },
      2023: {
        value: "E 857,100.00",
        trend:
          '<i class="fas fa-arrow-down trend-negative"></i> -5.2% from 2022',
        forecast: "E 820,000.00",
        yoyGrowth: -5.2,
      },
      all: {
        value: "E 1,849,600.00",
        trend: "Covering all available time",
        forecast: "E 2,100,000.00",
        yoyGrowth: 0,
      },
    },
    entities: {
      ...
      last: {
        value: "4",
        trend: '<i class="fas fa-minus"></i> No change from previous quarter',
        yoyGrowth: 0,
      },
      ytd: {
        value: "8",
        trend:
          '<i class="fas fa-arrow-up trend-positive"></i> +4 new this year',
        yoyGrowth: 100,
      },
    },
  };
  ...
  showFallbackCharts() {
    ...
    containers.forEach((selector) => {
      ...
      if (container) {
        container.innerHTML =
          '<p style="text-align: center; padding: 2rem; color: #64748b;">Chart data will be loaded shortly...</p>';
      }
    });
  }
  ...
  updateMetric(metricId, filter) {
    ...
    if (!data) {
      console.warn(
        `No data found for metric '${metricId}' with filter '${filter}'`,
      );
      return;
    }
    ...
    const trendElement = metricCard.querySelector(
      ".card-body .trend-indicator",
    );
  ...
  createSparkline(element) {
    ...
    const sparklineId =
      element.id ||
      `sparkline-${[...Array(5)].map(() => Math.random().toString(36)[2]).join("")}`;
    if (!element.id) {
  ...
  toggleWidget(widgetName) {
    this.dashboardConfig.widgets[widgetName] =
      !this.dashboardConfig.widgets[widgetName];
  ...
  updateWidgetVisibility() {
    Object.entries(this.dashboardConfig.widgets).forEach(
      ([widget, visible]) => {
