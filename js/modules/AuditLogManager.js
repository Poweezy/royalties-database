...
export class AuditLogManager {
  ...
  applyFilters() {
    ...
    this.filteredLogs = this.logs.filter(log => {
        ...
        const actionMatch = !action || log.action.toLowerCase().replace(' ', '-') === action;

        // Basic date filtering for demo purposes
        const dateMatch = !date || (date === 'today' && new Date(log.timestamp).toDateString() === new Date().toDateString());
  ...
  async generateMockData() {
    ...
    const actions = ['Login', 'Logout', 'Failed Login', 'Create User', 'Modify User', 'Delete User', 'Data Access'];
    const statuses = ['Success', 'Failed', 'Success', 'Success', 'Success', 'Success', 'Success'];
    const agents = ['Chrome 121.0', 'Firefox 123.0', 'Safari 17.2', 'Edge 121.0', 'Unknown'];

    for (let i = 0; i < 50; i++) {
        const userIndex = Math.floor(Math.random() * users.length);
        const actionIndex = Math.floor(Math.random() * actions.length);
        const agentIndex = Math.floor(Math.random() * agents.length);
        const status = (actions[actionIndex] === 'Failed Login') ? 'Failed' : statuses[actionIndex];
        const date = new Date();
