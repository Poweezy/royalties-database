...
export class GisDashboard {
    ...
    init() {
        ...
        const layerControl = L.control.layers(this.baseLayers, this.overlayMaps).addTo(this.map);
    ...
    addMineMarkers(mines) {
        ...
        this.mineLocations.forEach(mine => {
            ...
            const marker = L.marker([mine.lat, mine.lon], { icon: icon })
    ...
    getMineralType(mineName) {
        if (mineName.toLowerCase().includes('coal') || mineName.toLowerCase().includes('colliery')) {
            return 'coal';
        } else if (mineName.toLowerCase().includes('iron') || mineName === 'Ngwenya Mine') {
            return 'iron';
        } else if (mineName.toLowerCase().includes('gravel') || mineName.toLowerCase().includes('quarry')) {
            return 'gravel';
        }
        return 'general';
    }
        ...
        const concessionData = [
            {
                ...
                coordinates: [
                    [-27.05, 31.60], [-26.98, 31.60], [-26.98, 31.70], [-27.05, 31.70], [-27.05, 31.60]
                ],
                type: "Coal Mining",
                status: "Active"
            },
            {
                ...
                coordinates: [
                    [-26.25, 30.98], [-26.15, 30.98], [-26.15, 31.08], [-26.25, 31.08], [-26.25, 30.98]
                ],
    ...
    loadDistricts() {
        const districtData = [
            {
                name: "Hhohho Region",
                coordinates: [
                    [-26.0, 31.0], [-25.8, 31.0], [-25.8, 31.3], [-26.0, 31.3], [-26.0, 31.0]
                ]
            },
            {
                name: "Manzini Region", 
                coordinates: [
                    [-26.3, 31.0], [-26.1, 31.0], [-26.1, 31.4], [-26.3, 31.4], [-26.3, 31.0]
                ]
            }
        ];
        ...
        districtData.forEach(district => {
            ...
            polygon.bindPopup(`<h5>${district.name}</h5><p>Administrative District</p>`);
        });
    }
    ...
    loadProtectedAreas() {
        const protectedAreas = [
            {
                ...
                coordinates: [
                    [-26.2, 31.05], [-26.1, 31.05], [-26.1, 31.15], [-26.2, 31.15], [-26.2, 31.05]
                ]
            }
        ];
        ...
        protectedAreas.forEach(area => {
            ...
            polygon.bindPopup(`<h5>${area.name}</h5><p>Protected Environmental Area</p>`);
        });
    }
    ...
    loadInfrastructure() {
        const infrastructureData = [
            {
                name: "Main Highway",
                coordinates: [[-26.5, 31.0], [-26.3, 31.2], [-26.1, 31.4]],
                type: "highway"
            },
            {
                name: "Railway Line",
                coordinates: [[-26.8, 31.1], [-26.5, 31.3], [-26.2, 31.5]],
                type: "railway"
            }
        ];
        ...
        infrastructureData.forEach(infra => {
            ...
            polyline.bindPopup(`<h5>${infra.name}</h5><p>Infrastructure: ${infra.type}</p>`);
        });
    }
    ...
    setupMeasurementTools() {
        ...
        measureControl.onAdd = (map) => {
            ...
            div.innerHTML = '<a href="#" title="Measure distances and areas"><i class="fas fa-ruler"></i></a>';
    ...
    setupDrawingTools() {
        ...
        drawControl.onAdd = (map) => {
            const div = L.DomUtil.create('div', 'leaflet-bar leaflet-control leaflet-control-custom');
            div.innerHTML = '<a href="#" title="Draw shapes and buffer zones"><i class="fas fa-draw-polygon"></i></a>';
            div.style.backgroundColor = 'white';
            div.style.width = '30px';
            div.style.height = '30px';
            div.style.lineHeight = '30px';
    ...
    setupCoordinateDisplay() {
        ...
        coordDisplay.onAdd = (map) => {
            ...
            div.innerHTML = '<span id="coord-text">Click on map for coordinates</span>';
    ...
    setupLocationControl() {
        ...
        locationControl.onAdd = (map) => {
            const div = L.DomUtil.create('div', 'leaflet-bar leaflet-control leaflet-control-custom');
            div.innerHTML = '<a href="#" title="Show my location"><i class="fas fa-crosshairs"></i></a>';
            div.style.backgroundColor = 'white';
            div.style.width = '30px';
            div.style.height = '30px';
            div.style.lineHeight = '30px';
    ...
    setupMapExport() {
        ...
        exportControl.onAdd = (map) => {
            const div = L.DomUtil.create('div', 'leaflet-bar leaflet-control leaflet-control-custom');
            div.innerHTML = '<a href="#" title="Export map"><i class="fas fa-download"></i></a>';
            div.style.backgroundColor = 'white';
            div.style.width = '30px';
            div.style.height = '30px';
            div.style.lineHeight = '30px';
    ...
    addEnhancedLegend() {
        ...
        legend.onAdd = () => {
            ...
            let labels = ['<h4><i class="fas fa-map-marked-alt"></i> Map Legend</h4>'];

            // Mining markers
            labels.push('<div class="legend-section"><h5>Mining Operations</h5>');
            labels.push(`<div class="legend-item"><img src="${this.icons.mine.options.iconUrl}" style="width: 16px; height: 24px;"> General Mine</div>`);
            labels.push(`<div class="legend-item"><img src="${this.icons.coalMine.options.iconUrl}" style="width: 16px; height: 24px;"> Coal Mine</div>`);
            labels.push(`<div class="legend-item"><img src="${this.icons.ironMine.options.iconUrl}" style="width: 16px; height: 24px;"> Iron Ore Mine</div>`);
            labels.push(`<div class="legend-item"><img src="${this.icons.quarry.options.iconUrl}" style="width: 16px; height: 24px;"> Quarry</div>`);
            labels.push(`<div class="legend-item"><img src="${this.icons.gravelQuarry.options.iconUrl}" style="width: 16px; height: 24px;"> Gravel Quarry</div></div>`);

            // Area overlays
            labels.push('<div class="legend-section"><h5>Areas & Boundaries</h5>');
            labels.push('<div class="legend-item"><span class="legend-polygon" style="background: rgba(255,107,53,0.3); border: 2px solid #ff6b35;"></span> Mining Concessions</div>');
            labels.push('<div class="legend-item"><span class="legend-polygon" style="background: rgba(37,99,235,0.1); border: 1px dashed #2563eb;"></span> Administrative Districts</div>');
            labels.push('<div class="legend-item"><span class="legend-polygon" style="background: rgba(22,163,74,0.3); border: 2px solid #16a34a;"></span> Protected Areas</div></div>');

            // Infrastructure
            labels.push('<div class="legend-section"><h5>Infrastructure</h5>');
            labels.push('<div class="legend-item"><span class="legend-line" style="background: #dc2626;"></span> Highways</div>');
            labels.push('<div class="legend-item"><span class="legend-line" style="background: #7c3aed;"></span> Railways</div></div>');
