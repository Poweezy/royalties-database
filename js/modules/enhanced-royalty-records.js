...
class EnhancedRoyaltyRecords {
    constructor() {
        ...
        this.paymentStatuses = ['Draft', 'Pending', 'Paid', 'Overdue', 'Disputed', 'Partially Paid'];
        this.calculationMethods = ['fixed', 'tiered', 'sliding_scale', 'ad_valorem', 'percentage'];
    ...
    initializeValidationRules() {
        return {
            ...
            entity: {
                required: true,
                type: 'string',
                validEntities: ['Kwalini Quarry', 'Maloma Colliery', 'Mbabane Quarry', 'Ngwenya Mine', 'Sidvokodvo Quarry']
            },
            mineral: {
                required: true,
                type: 'string',
                validMinerals: ['Coal', 'Iron Ore', 'Quarried Stone', 'Gravel', 'Diamond', 'Gold']
            },
            paymentDate: {
                required: true,
                type: 'date',
    ...
    bindEvents() {
        ...
        this.elements.templateBtn?.addEventListener('click', (e) => this.saveAsTemplate(e));
        this.elements.bulkBtn?.addEventListener('click', (e) => this.showBulkCreateDialog(e));
        
        // Real-time validation and calculation
        this.elements.volumeInput?.addEventListener('input', () => this.validateAndCalculate());
        this.elements.tariffInput?.addEventListener('input', () => this.validateAndCalculate());
        this.elements.entitySelect?.addEventListener('change', () => this.onEntityChange());
        this.elements.mineralSelect?.addEventListener('change', () => this.onMineralChange());
        this.elements.currencySelect?.addEventListener('change', () => this.validateAndCalculate());
        this.elements.calculationMethodSelect?.addEventListener('change', () => this.onCalculationMethodChange());
        this.elements.contractSelect?.addEventListener('change', () => this.onContractChange());
    ...
    validateField(fieldName, value) {
        ...
        if (rule.type === 'date') {
            ...
            if (isNaN(date.getTime())) {
                ...
                if (rule.maxFutureDays) {
                    ...
                    if (date > maxDate) {
                        errors.push(`${fieldName} cannot be more than ${rule.maxFutureDays} days in the future`);
                    }
                }
            }
        }
        ...
        if (rule.validEntities && !rule.validEntities.includes(value)) {
            errors.push(`${fieldName} must be one of: ${rule.validEntities.join(', ')}`);
        }

        if (rule.validMinerals && !rule.validMinerals.includes(value)) {
            errors.push(`${fieldName} must be one of: ${rule.validMinerals.join(', ')}`);
        }

        if (rule.validCurrencies && value && !rule.validCurrencies.includes(value)) {
            errors.push(`${fieldName} must be one of: ${rule.validCurrencies.join(', ')}`);
        }
    ...
    validateBusinessRules(formData) {
        ...
        if (formData.entity === 'Small Scale Mining' && formData.volume > 10000) {
            errors.push('Small scale mining operations cannot exceed 10,000 tons per period');
        }
        ...
        if (range && (formData.tariff < range.min || formData.tariff > range.max)) {
            errors.push(`Tariff for ${formData.mineral} should be between ${range.min} and ${range.max}`);
        }
    ...
    validateDataIntegrity(formData) {
        ...
        if (this.isDuplicateRecord(formData)) {
            errors.push('A similar record already exists for this entity, mineral, and period');
        }
        ...
        if (Math.abs(formData.royaltyPayment - expectedPayment) > 0.01) {
            errors.push(`Calculated payment (${expectedPayment.toFixed(2)}) doesn't match entered payment`);
        }
    ...
    calculateRoyaltyPayment(formData) {
        ...
        if (formData.status === 'Overdue') {
            calculations.penalties = this.calculatePenalties(formData, calculations.base);
        }

        // Interest calculations for late payments
        if (this.isPaymentLate(formData)) {
            calculations.interest = this.calculateInterest(formData, calculations.base);
        }

        // Currency adjustments
        if (formData.currency && formData.currency !== 'SZL') {
            calculations.adjustments = this.applyCurrencyAdjustment(calculations.base, formData.currency);
        }

        calculations.total = calculations.base + calculations.penalties + calculations.interest + calculations.adjustments;
    ...
    calculateTieredRoyalty(formData) {
        ...
        if (contract && contract.calculationParams?.tiers) {
            return this.calculator.calculateTiered(contract, { volume: formData.volume });
        }
        return formData.volume * formData.tariff;
    }
    ...
    calculateAdValoremRoyalty(formData) {
        ...
        const marketValue = formData.marketValue || (formData.volume * formData.commodityPrice);
    ...
    calculatePercentageRoyalty(formData) {
        ...
        const grossValue = formData.grossValue || (formData.volume * formData.unitPrice);
        const rate = formData.percentageRate || 0.10; // 10% default
    ...
    async updatePaymentStatus(recordId, newStatus, partialAmount = 0) {
        ...
        if (newStatus === 'Partially Paid') {
            ...
            record.remainingAmount = record.royaltyPayment - this.getTotalPartialPayments(record);
        }
    ...
    async loadTemplate() {
        ...
        Object.keys(template.data).forEach(key => {
            const element = this.elements[key + 'Input'] || this.elements[key + 'Select'];
            if (element) {
    ...
    displayValidationResults(errors) {
        ...
        if (errors.length === 0) {
            this.elements.validationSummary.innerHTML = '<span class="text-success">✓ All validations passed</span>';
    ...
    getFormData() {
        return {
            ...
            calculationMethod: this.elements.calculationMethodSelect?.value || 'fixed',
    ...
    async loadTemplates() { /* Implementation for loading saved templates */ }
    async saveTemplates() { /* Implementation for saving templates */ }
    async populateTemplateSelect() { /* Implementation for populating template dropdown */ }
    isDuplicateRecord(formData) { /* Implementation for duplicate checking */ return false; }
    calculateExpectedPayment(formData) { /* Implementation for expected payment calculation */ return 0; }
    checkContractCompliance(formData, contract) { /* Implementation for contract compliance */ return { errors: [] }; }
    getTotalPartialPayments(record) { /* Implementation for calculating total partial payments */ return 0; }
    async logStatusChange(record, oldStatus, newStatus) { /* Implementation for audit logging */ }
    async matchBankRecord(record) { /* Implementation for bank record matching */ return null; }
    async reconcilePayment(record, bankRecord) { /* Implementation for payment reconciliation */ return false; }
    async createNotification(notification) { /* Implementation for creating notifications */ }
    async setupNotificationScheduler() { /* Implementation for setting up notification scheduler */ }
    onEntityChange() { /* Implementation for entity change handler */ }
    onMineralChange() { /* Implementation for mineral change handler */ }
    onCalculationMethodChange() { /* Implementation for calculation method change handler */ }
    onContractChange() { /* Implementation for contract change handler */ }
    showBulkCreateDialog(event) { /* Implementation for bulk create dialog */ }
    applyFilters() { /* Implementation for applying filters */ }
    async renderRecords(filter = null) { /* Implementation for rendering records */ }
    exportRecords() { /* Implementation for exporting records */ }
}
